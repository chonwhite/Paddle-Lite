...
void QuantDequantOpFuser::InsertNewNode(SSAGraph* graph,
  ...
  for (int i = 0; i < times_; i++) {
    ...
    // LOG(INFO) << temp_tensor;
    // change the weight from the float type to int8 type.
    // std::cout << "whole_weight_scale:" << whole_weight_scale << " max_range:"
    // << max_range << std::endl;
    // float max_value = (127 * 127) / max_range;
    // float scale = max_value / 127;
    // std::cout << "max_value:" << max_value  << "scale:::" << scale <<
    // std::endl;
